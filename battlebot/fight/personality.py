...
No magic/energy equalization by default. Under this simulation's configured
rules, genjutsu requires target chakra; Stands require a Stand user to perceive
or harm them. Unknown prerequisites cannot be assumed true. These are simulation
rules, not claims that all fictional universes follow the same canon.
...
def parse_phases(raw):
    matches = list(re.finditer(r"(?im)^\s*(?:\*\*)?Phase ([123])\s*[:\-].*$", raw))
...
def details(decision, packet=None):
    ...
    if len(phases) != 3:
        phases = ["A complete matchup-specific assessment is unavailable for this phase. Review Quick Evidence; no battle sequence is being asserted."] * 3
